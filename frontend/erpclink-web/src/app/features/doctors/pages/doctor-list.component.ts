...
import { DoctorListItemDto } from '../models/doctor.models';
...
export class DoctorListComponent implements OnInit {
  ...
  readonly page = signal<PagedResult<DoctorListItemDto> | null>(null);
  ...
  async toggleActive(item: DoctorListItemDto): Promise<void> {
    const activate = !item.isActive;
    const ok = await this.confirm.confirm({
      title: activate ? 'تفعيل الطبيب' : 'إيقاف الطبيب',
      message: `${activate ? 'تفعيل' : 'إيقاف'} ${item.displayName}؟`,
