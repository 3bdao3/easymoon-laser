...
import { ClinicListItemDto } from '../models/clinic.models';
...
export class ClinicListComponent implements OnInit {
  ...
  readonly page = signal<PagedResult<ClinicListItemDto> | null>(null);
  ...
  ngOnInit(): void {
    this.load();
    this.filters.controls.query.valueChanges.pipe(debounceTime(350), distinctUntilChanged()).subscribe(() => {
      this.currentPage = 1;
      this.load();
    });
  ...
  load(): void {
    ...
    const isActive = isActiveRaw === 'all' ? undefined : isActiveRaw === 'true';
    this.api
      .search({
  ...
  async toggleActive(item: ClinicListItemDto): Promise<void> {
    const activate = !item.isActive;
    const ok = await this.confirm.confirm({
      title: activate ? 'تفعيل العيادة' : 'إيقاف العيادة',
