...
@Component({
  selector: 'app-ar-overview',
  standalone: true,
  imports: [PageHeaderComponent, LoadingSpinnerComponent, FormsModule, RouterLink],
  template: `
    <app-page-header title="الذمم المدينة (AR)" subtitle="Accounts receivable subledger" />
    ...
    <form class="feature-form" (ngSubmit)="load()">
      <input [(ngModel)]="partyId" name="partyId" placeholder="معرّف العميل (PatientId)" required />
...
export class ArOverviewComponent {
  ...
  load(): void {
    ...
    this.error.set(null);
    this.api.getArBalance(id).subscribe({
      next: (b) => {
        this.balance.set(b);
        this.api.getArStatement(id).subscribe({ next: (s) => this.statement.set(s) });
        this.api.searchArTransactions(id).subscribe({ next: (r) => this.transactions.set(r.items) });
        this.api.getArAging(id).subscribe({
          next: (a) => {
            this.aging.set(a);
  ...
  private fail(err: unknown): void {
    this.loading.set(false);
    this.error.set('تعذر تحميل بيانات AR');
    console.error(err);
  }
}
